...
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Sora:wght@400;600;700;800&family=Playfair+Display:wght@700;900&display=swap');
...
[data-testid="stSidebar"]{background:#fff!important;border-right:1px solid #E4E9F2}
.stButton>button{font-family:'Sora',sans-serif;font-weight:700;border-radius:10px;transition:all .18s}
.stButton>button[kind="primary"]{background:linear-gradient(135deg,#0F9B58,#13C46E);border:none;color:white}
[data-testid="stMetric"]{background:white;border:1px solid #E4E9F2;border-radius:14px;padding:1rem 1.25rem;box-shadow:0 2px 10px rgba(11,31,74,.06)}
[data-testid="stMetricLabel"]{font-weight:700;font-size:12px;text-transform:uppercase;letter-spacing:.05em;color:#64748B}
[data-testid="stMetricValue"]{font-family:'Playfair Display',serif;font-size:2rem;font-weight:900;color:#0B1F4A}
.stTabs [data-baseweb="tab"]{border-radius:8px;font-weight:700;font-size:14px}
.stTabs [aria-selected="true"]{background:#E6F9F0;color:#0F9B58}
.hero{background:linear-gradient(155deg,#0B1F4A 0%,#152D6E 60%,#1A4A9F 100%);border-radius:22px;padding:56px 48px;color:white;margin-bottom:32px}
.hero h1{font-family:'Playfair Display',serif;font-size:3rem;font-weight:900;color:white;margin-bottom:12px}
.hero p{font-size:1.05rem;color:rgba(255,255,255,.7);line-height:1.7}
.tc{background:white;border:1px solid #E4E9F2;border-radius:18px;padding:22px;box-shadow:0 2px 10px rgba(11,31,74,.06);margin-bottom:8px}
.sc{background:white;border:1px solid #E4E9F2;border-radius:16px;padding:20px;box-shadow:0 2px 10px rgba(11,31,74,.06);margin-bottom:12px}
.nv{background:linear-gradient(135deg,#0B1F4A,#152D6E);border-radius:18px;padding:32px;color:white;margin-bottom:16px}
</style>
...
# ── SIDEBAR ───────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("<div style='text-align:center;padding:12px 0 20px'><span style='font-size:44px'>🛡️</span><br><span style='font-size:22px;font-weight:900;color:#0B1F4A'>SomaliTutor</span><br><span style='font-size:11px;color:#64748B'>Verified Teachers Worldwide</span></div>", unsafe_allow_html=True)
    st.divider()
    NAVS=["🏠  Home","🔍  Find a Tutor","🤖  AI Matching","📅  My Bookings","📊  Progress","💳  Pricing","🔐  Login","✍️  Sign Up","👤  My Account","🎓  Teacher Hub","⚙️  Admin"]
    idx=NAVS.index(st.session_state.nav) if st.session_state.nav in NAVS else 0
    nav=st.radio("Nav",NAVS,index=idx,label_visibility="collapsed")
    ...
    st.divider()
    st.caption("📞 support@somalitutor.com")
...
def page_home():
    ...
    for i,t in enumerate(sorted(TEACHERS,key=lambda x:-x["rating"])[:3]):
        with cols[i]:
            ...
            if st.button(f"Book {t['name'].split()[0]}",key=f"h_{t['id']}",use_container_width=True,type="primary"):
